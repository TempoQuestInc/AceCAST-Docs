...
`acecast-v3.0.1/acecast/run/` directory.

For this example we will assume your AceCAST installation is in your home directory (i.e. at 
`~/acecast-v3.0.1`). If it is somewhere else you will need to modify the code examples accordingly.
...
necessary runtime and input files. This will typically include the following:
    ...
    - AceCAST Run Files (always found in `~/acecast-v3.0.1/acecast/run/`):
    ...
    - Simulation Specific Input Data and Configuration Files (found in `~/acecast-v3.0.1/acecast/benchmarks/easter500` for this example):
...
`~/acecast-v3.0.1/acecast/easter500-4GPU`:

.. code-block:: shell

    # Create and cd to new run directory
    mkdir ~/acecast-v3.0.1/acecast/easter500-4GPU
    cd ~/acecast-v3.0.1/acecast/easter500-4GPU
